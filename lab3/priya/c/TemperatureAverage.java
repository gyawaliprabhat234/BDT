...
public class TemperatureAverage extends Configured implements Tool{
	...
	Mapper<LongWritable, Text, Text, TemperatureWritable>{
		...
		private Map<String, TemperatureWritable> dateMapper;
		...
		public void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException{
			String date = value.toString().substring(15, 19);
			Double temp = Double.parseDouble(value.toString().substring(87, 92))/10.0;
			double count = 1;
			if(dateMapper.containsKey(date)){
				TemperatureWritable previousVal = dateMapper.get(date);
				temp += previousVal.getSum();
				count += previousVal.getCount();
				
			}
			dateMapper.put(date, new TemperatureWritable(temp, count));
		...
		protected void cleanup(Context context) throws IOException, InterruptedException {
			// TODO Auto-generated method stub
			Set<String> yearSet = dateMapper.keySet();
			for (String year: yearSet)
			{
				date.set(year);
				context.write(date, dateMapper.get(year));
			}
		}
